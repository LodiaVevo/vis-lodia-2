...
exports.run = async (client, message, args) => {
  ...
  if(!users) return message.channel.send(embedx.setDescription(`Bir Üye Etiketlemelisin.`).setFooter("Lodia ♥ Lodia").setTimestamp().setColor("RED"))
...
users.roles.remove(r.id)
  ...
  message.channel.send(embed.setDescription(`${users} Adlı Kullanıcı Başarıyla Kayıtsız'a Atıldı.`).setTimestamp().setColor("RED").setFooter("discord.gg/striga"))
}
module.exports.conf = {
  enabled: true,
  guildOnly: true,
